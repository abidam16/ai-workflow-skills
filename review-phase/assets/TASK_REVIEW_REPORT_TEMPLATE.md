# Task Review Report

## 1. Review Metadata

- Review mode: `TASK_REVIEW`
- Review target:
- Plan reference:
- Implementation reference:
- Reviewer:
- Date:

## 2. Source Artifacts Used

| Artifact | Path / Reference | Status | Used For |
|---|---|---|---|
| PLAN | | | task scope and execution contract |
| PRD | | | product behavior / acceptance intent |
| Architecture | | | system-shape constraints |
| ADR | | | recorded technical decisions |
| Roadmap | | | phase/slice context |
| Implementation Summary | | | delivered changes and deviations |
| Validation Evidence | | | tests/checks/manual evidence |

## 3. Source Artifact Readiness

- Missing artifacts:
- Stale or weak artifacts:
- Conflicting artifacts:
- Review impact:

## 4. Executive Verdict

- Final status: `APPROVED | APPROVED_WITH_MINOR_IMPROVEMENTS | NEEDS_REVISION | BLOCKED`
- Acceptance summary:
- Blocking issue count:
- Non-blocking issue count:
- Main reason for status:

## 5. Business / Product Alignment

- Assessment: `PASS | PARTIAL | FAIL | NOT_APPLICABLE`
- Summary:
- Evidence:
- Gaps:

## 6. Architecture Alignment

- Assessment: `PASS | PARTIAL | FAIL | NOT_APPLICABLE`
- Architecture scope reviewed: `ROOT | INITIATIVE | BOTH | NONE`
- Architecture path(s):
- Constraints checked:
- Evidence:
- Gaps:

## 7. ADR / Decision Alignment

- Assessment: `PASS | PARTIAL | FAIL | NOT_APPLICABLE`
- ADRs reviewed:
- Decisions checked:
- Evidence:
- Gaps:
- New ADR needed: `YES | NO`

## 8. Roadmap Alignment

- Assessment: `PASS | PARTIAL | FAIL | NOT_APPLICABLE`
- Roadmap slice reviewed:
- Exit criteria checked:
- Evidence:
- Gaps:

## 9. Plan Alignment

- Assessment: `PASS | PARTIAL | FAIL`
- Objective fulfilled:
- In-scope items completed:
- Out-of-scope respected:
- Expected changes completed:
- Deviations:

## 10. Technical Quality

- Assessment: `PASS | PARTIAL | FAIL`
- Strengths:
- Risks:
- Maintainability concerns:
- Integration concerns:

## 11. Validation and Test Adequacy

- Assessment: `PASS | PARTIAL | FAIL`
- Tests run or updated:
- Manual validation:
- Evidence quality:
- Remaining validation gaps:

## 12. Findings

| Severity | Category | Title | Description | Why It Matters | Recommended Action |
|---|---|---|---|---|---|
| `HIGH_URGENCY | MEDIUM_URGENCY | LOW_URGENCY | FUTURE_IMPROVEMENT` | | | | | |

## 13. Risk Assessment

- Acceptance risk:
- Release risk:
- Architecture drift risk:
- Follow-up risk:

## 14. Recommended Next Actions

List supporting actions here, ordered by importance. This section may contain multiple recommendations, but the final `Concrete Next Step` section must choose exactly one immediate action.

1.
2.
3.

## 15. Concrete Next Step

- `next_step_type`: `MERGE_OR_CLOSE_TASK | APPLY_MINOR_FIXES | RETURN_TO_IMPLEMENTATION | UPDATE_PLAN | UPDATE_LIGHTWEIGHT_PLAN | UPDATE_ARCHITECTURE | CREATE_ADR | UPDATE_ADR | UPDATE_ROADMAP | UPDATE_PRD | REQUEST_MISSING_EVIDENCE | START_NEXT_PLAN | ESCALATE_TO_FULL_WORKFLOW | STOP_AND_ESCALATE`
- `target`:
- `action`:
- `why_this_is_next`:
- `blocking_condition`:
- `suggested_prompt`:
