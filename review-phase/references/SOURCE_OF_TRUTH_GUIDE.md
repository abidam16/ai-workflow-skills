...
| Review Report | acceptance decision, findings, and one concrete next step | creating upstream artifact truth |
...
If the plan says to store authorization state in a notification table, but architecture says membership is the authorization source of truth, the review must not approve.

Route to `UPDATE_PLAN` or `UPDATE_ARCHITECTURE`, depending on which artifact is wrong.
